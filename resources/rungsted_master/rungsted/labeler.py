...
if args.confusion_scaling:
    ...
    #logging.info("Confusion scaling. Matrix specifies {} overlapping labels with mean scaling factor {:.3f}".format(len(set(labels) & set(confusion_scaling_pd.index)),confusion_scaling.mean()))
...
def do_train(transition, emission):
    ...
    for epoch in range(1, args.passes+1):
        for sent in train:
            ...
            if n_updates % 1000 == 0:
                continue#print >>sys.stderr, '\r[{}] {}k sentences total'.format(epoch, n_updates / 1000),
    ...
    #print >>sys.stderr, "Training took {:.2f} secs. {} words/sec".format(timers['train'].elapsed(),int(tokens_trained / timers['train'].elapsed()))
...
def do_test(transition, emission):
    ...
    with open(args.predictions or os.devnull, 'w') as out:
        ...
        for line in test_data:
            words.append([w for w in line.strip().split() if w.startswith('w')])
        for sent_i, sent in enumerate(test):
            ...
            for example_id, gold_label, pred_label in zip(sent.ids, sent.gold_labels, sent.pred_labels):
                word = ''.join(words[count])
                print >>out, "{}\t{}\t{}\t{}".format(example_id, word[2:], labels_map[gold_label], labels_map[pred_label])
                count += 1


    timers['test'].end()
    #logging.info("Accuracy: {:.3f}".format(accuracy(test)))
    #print >>sys.stderr, "Test took {:.2f} secs. {} words/sec".format(timers['test'].elapsed(),int(sum(len(seq) for seq in test) / timers['test'].elapsed()))
    ...
    print "* * * * * * * * * >>  Accuracy: {:.1f} on test {} ".format((100*accuracy(test)),args.test)
